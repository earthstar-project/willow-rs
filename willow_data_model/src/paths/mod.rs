...
impl core::fmt::Display for PathFromComponentsError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            PathFromComponentsError::PathTooLong => {
                write!(
                    f,
                    "Total length of a path in bytes exceeded the maximum path length"
                )
            }
            PathFromComponentsError::TooManyComponents => {
                write!(
                    f,
...
impl std::error::Error for PathFromComponentsError {}
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            PathError::ComponentTooLong => {
                write!(
                    f,
                    "Length of a path component exceeded the maximum component length"
                )
            }
        }
    }
...
impl<const MCL: usize, const MCC: usize, const MPL: usize> Path<MCL, MCC, MPL> {
    ...
    pub fn total_length_of_prefix(&self, i: usize) -> usize {
        Representation::total_length(&self.data, i)
    }
...
impl<const MCL: usize, const MCC: usize, const MPL: usize> Successor for Path<MCL, MCC, MPL> {
    ...
    ///     [].as_slice(),
    /// ])?;
    /// assert_eq!(p.successor(), Some(Path::from_slices(&[
    ///     [255].as_slice(),
    ///     [10].as_slice(),
    /// ])?));
    /// # Ok::<(), PathError>(())
