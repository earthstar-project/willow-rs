...
where
    StoreCreationFunction: AsyncFn(&N) -> Result<Store, CreationError>,
    N: Eq + Hash + Clone,
{
    /// Get (and if necessary first create) the store for entries of the given namespace_id.
    pub async fn get_store(&self, namespace_id: &N) -> Result<Rc<Store>, CreationError> {
        ...
        match map_ref.get(namespace_id) {
            ...
            None => {
                map_ref.insert(
                    namespace_id.clone(),
                    Rc::new((self.fun)(namespace_id).await?),
                );
                return self.get_store(namespace_id).await;
            }
        }
    }
}
